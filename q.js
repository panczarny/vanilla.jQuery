...
(function() {
  ...
  const _helpers = {
    ...
    matches: function(node, selector) {
      if(typeof node !== 'object' || node.nodeType !== NODES_TYPES.ELEMENT_NODE) {
        return false;
      }
    ...
    doANIMfade: function(type, duration = DEFAULT_DURATION, callback = function() {}, display = 'block') {
  ...
  Q.fn.extend({
    ...
    empty: function() {
      this.each((node) => {
        if(node.nodeType === NODES_TYPES.ELEMENT_NODE) {
    ...
    parent: function() {
      ...
      retVal = parentElement.nodeType === NODES_TYPES.ELEMENT_NODE ? Q(parentElement) : null;
      return retVal;
    },
    ...
    parents: function(selector) {
      ...
      this.each((node) => {
        let parent = node;
        while((parent = parent.parentNode) && parent.nodeType == NODES_TYPES.ELEMENT_NODE) {
          if(_helpers.matches(parent, selector) && !foundParents.includes(parent)) {
    ...
    trigger: function(eventName) {
      ...
      const findDocument = function(node) {
        ...
        } else if (node.nodeType == NODES_TYPES.DOCUMENT_NODE_TYPE){
          doc = node;
        } else {
          doc = false;
        }
  ...
  Q.extend({
    ...
    isElement: (element) => {
      const allowedNodeTypes = [NODES_TYPES.ELEMENT_NODE, NODES_TYPES.DOCUMENT_NODE_TYPE, NODES_TYPES.DOCUMENT_FRAGMENT_NODE_TYPE];
